...
get_circuits <- function(
  ...
  iter <- ifelse(part == 1, connections, length(combs))
  for (i in 2:iter) {
    if (verbose) {message(
        "Connection ", i, ":\n(", 
        dists_ord[i,"b1_ind"], ") ", 
    ...
    if (part == 2) {
      ...
      if (length(unconnected_boxes) == 0 & length(circuits) == 1) {
        return(
          as.numeric(input[dists_ord[i,"b1_ind"], 1]) * 
...
# part 1
get_circuits(part=1, verbose=TRUE)
get_circuits("input.txt", part=1, connections=1000)

# part 2
get_circuits(part=2, verbose=TRUE)
get_circuits("input.txt", part=2)
